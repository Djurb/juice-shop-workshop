...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  filterTable () {
    ...
    if (queryParam) {
      ...
      // Use sanitize() instead of bypassSecurityTrustHtml to prevent XSS
      this.searchValue = this.sanitizer.sanitize(1, queryParam) // vuln-code-snippet vuln-line localXssChallenge xssBonusChallenge
